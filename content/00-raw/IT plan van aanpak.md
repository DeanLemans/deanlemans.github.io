---
title: IT ondernemings plan
tags:
  - research
publish: false
modified: 2026-05-03
id: 01KQQV6TWR33FATQZE504ND60T
created: 2026-03-17
aliases:
  - IT ondernemings plan
---

# IT ondernemings plan

random shit:
personal subscription? services for older people who dont know tech.
maby just normalal selling?
helping with computers and such

business plan:
how:
advertise via facebook as most old people are on there
what:
a service that could/can be a subscrition that aims to help old people with interner related stuff(like help with laptorp, phone, or tv)

---

how: sell/advertise via facebook for huiservice
what: windows iso, office software, en huiservice

barebones:
adverteer op facebook dat ik met it kan helpen, op het begin gefocused op oudere groep mensen omdat hun nog meer hulp nodig hebben.

tariefen:
best effort, no data liability
gratis check(naar locatie), op basis van probleem geef ik een prijs.
afrekenen: contant of via betaalverzoek/tikkie

future services?:
Easy
- PC and laptop setup
- Installing and configuring software
- System cleanup and optimization
- Hardware installation

Medium
- Remote desktop troubleshooting
- Troubleshooting IT problems
- Linux installation

Moderate
- IT purchasing advice
- Technology recommendations

---

# IT Ondernemingsplan - Dean De Dean

**Naam bedrijf:** idk

Ik ben een IT-student en bied simpele, betaalbare computerhulp aan huis.
Geen ingewikkelde contracten, gewoon helpen met dagelijkse IT-problemen.

## Hoe klanten?

- Adverteren via Facebook (Marktplaats)
- Locale Facebookgroepen
- Vooral hulp aan huis
- Focus op oudere mensen, omdat zij vaak extra hulp nodig hebben

## Wat doe ik?

- Software installeren en instellen (zoals Windows, Office en andere programma's)
- Computerhulp aan huis bij dagelijkse problemen
- Problemen oplossen waar mensen zelf niet uitkomen (traag systeem, foutmeldingen, iets dat "niet meer werkt")
- Uitleg geven en helpen bij basisgebruik van software

## Hoe werk ik?

- Ik kom langs voor een gratis check
  - Max ±20 minuten
  - Alleen diagnose
- Geen fixes tijdens check
- Ik kijk wat het probleem is
- Daarna spreek ik een vaste prijs af
- Alles op basis van best effort
- Geen verantwoordelijkheid voor dataverlies

## Betaling

- Contant
- Betaalverzoek / Tikkie

## Na de klus

- Vraag of ze je willen aanraden
- Laat een briefje/kaartje achter met je naam + nummer
- Eigen kaartje maken kan handig zijn

---

## Possible services

- PC en laptop klaarmaken/instellen
- Software installeren en instellen
- Computer opschonen en sneller maken
- Hardware installeren (SSD, RAM, printer)
- Linux installeren
- Windows installeren

## Misschien in de toekomst

- Remote desktop hulp
- PC en laptop instellen met basis software en verkopen
- Advies bij het kopen van een nieuwe pc/laptop
- Technologisch advies in begrijpelijke taal

---

## scale

- Maak een website
- Vraag tevreden klanten om je aan te raden
- Laat je naam en nummer achter na een klus
- Bied af en toe een herhaalcheck aan (niet opdringen)
- Schakel iemand in bij drukte

## Legal stuff

- Inschrijving: Kamer van Koophandel (eenmanszaak / ZZP)
- BTW-nummer
- (GDPR)
- NDA
