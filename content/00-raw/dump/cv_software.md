---
title: CV Software
publish: true
modified: 2026-04-22
created: 2025-09-25
id: 01KQQV6TSWH8HTEEJDJE9ZC3K0
---

# CV Software

(Internet CV, idk)
> [!danger] WIP
> Currently reworking this
> I need to split my 2 CV, maby do something with automation for companies?

> [!info]- Click here for PDF
> ***[PDF Version](<../../static_files/CV-DeanLemans.pdf>)***

---

> [!info] Introduction
> - Hello, I am Dean and I'm an indie Software Developer! 👋
> - I am experienced in Godot and Static Site Generators.
> - Looking for freelance projects.
> - My site is built with [Quartz v4](https://github.com/jackyzha0/quartz), a modern static site generator for personal knowledge management and publishing.
> - Deployment is automated using a [GitHub Actions workflow](https://github.com/DeanLemans/deanlemans.github.io/blob/v4/.github/workflows/deploy%20github%20pages.yml) that builds Quartz and publishes it directly to GitHub Pages on every push.

---

> [!info] Developer Experience
> - Intermediate experience in [Godot](https://godotengine.org/).
> - Advanced experience in Static Site Generators (Hugo, Jekyll, Quartz, 11ty).
> - Basic experience in [Git](https://git-scm.com/).
> - Advanced experience in [GitHub](https://github.com/DeanLemans).
> - Intermediate experience in [Zed](https://zed.dev/).
> - Intermediate experience in CI/CD (GitHub Actions).

> [!info] Work Experience
> - Shelf-Stocker *(2 years)*
> - Indie Software Developer *(current)*
>   - Building automation tools and static sites.
>   - Maintaining automated deployment pipelines for Quartz projects.

> [!info] Educations
> - [Udens College](https://www.udenscollege.nl/)
> - Software Development at [SintLucas](https://www.sintlucas.nl/)

---

> [!info] Projects
> - [[Finished-Projects]]
> - [GitHub Repositories](https://github.com/DeanLemans?tab=repositories)
> - Quartz-based portfolio (this site): Built with Quartz and deployed via GitHub Actions, ensuring automated CI/CD publishing to GitHub Pages."

> [!info] Languages
> - Dutch *(Native)*
> - English *(Advanced)*

## Repositories Strategy

- **GitHub:** Public work and portfolio.
- **Radicle:** Decentralized backup.
- Mirror strategy ensures all code is available and versioned across platforms.

---

> [!info] Contact me!
> - See my **[Contact Links](https://linksta.cc/@Dean)**
