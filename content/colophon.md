---
title: Colophon
tags:
  - exclude
modified: 2026-05-03
id: 01KQQV6TQNFNF7TE8MZ7YKAHAZ
created: 2026-04-30
---

# Colophon

'01-projects'
'02-digital garden'

this site is made with obsidian and quartz
see [vault_rules](<./vault_rules.md>) for exact details, this page is for explaining on how the site itself is build

truth is that i dont know the exact details, i use a SSG named Quartz, the reason for that is that i use obsidian as my notes editor, it has specific integrations for features that obsidian has.
but recently i have been thinking of moving to emacs instead of obsidian, but im getting sidetracked.

i write in markdown, quartz parses those file with node.js to html + css + js to the web, i use 3 github CI scripts to publish my site site to nekoweb, neocities, and github pages(also internet archive, but there i could just paste my link in)